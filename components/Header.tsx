    ...
    return (
        <header id='header' ref={menuRef} className="max-md:shadow-header w-full fixed max-md:bottom-0 md:top-0 left-0 z-50 bg-container-color md:bg-body-color">
            <nav className="max-w-4xl px-4 md:mx-auto h-14 md:h-16 flex justify-between items-center gap-4 bg-inherit">
                <abbr title='Change Theme' onClick={()=>setTheme(theme === 'light' ? "dark" : "light")} className="text-title-color font-medium hover:text-title-color-dark md:cursor-pointer no-underline">s2sharpit</abbr>

                <div className={`${menu ? "bottom-0" : "-bottom-full"} fixed md:static left-0 w-full md:w-fit bg-body-color pt-8 md:pt-0 px-2 sm:px-6 md:px-0 pb-16 md:pb-0 shadow-header md:shadow-none rounded-t-3xl md:rounded-none transition-all md:transition-none duration-300`}>
                    ...
                    <span className="md:hidden"><Close onClick={()=>setMenu(false)} fontSize="large" className='absolute right-4 bottom-3 text-title-color hover:text-title-color-dark p-1' /></span>
                </div>
